  ...
  >
    ...
    >
      <div class="mb-4 flex items-start justify-between gap-4">
        <h2 id="error-detail-title" class="text-[16px] font-semibold text-text-primary">
          Scan error
        </h2>
        <button
          type="button"
      ...
      <p class="mb-1 text-[12px] font-semibold tracking-[0.32px] text-text-secondary uppercase">
        Path
      </p>
      ...
      <p
        class="mb-6 max-h-[240px] overflow-y-auto rounded bg-neutral-bg p-3 font-mono text-[13px] leading-[18px] break-all text-text-primary"
      >
        {{ message }}
      </p>
      ...
      <div class="flex justify-end">
        <Button variant="outline" size="medium" @click="copy">
          <template #icon
            ><Icon :name="copied ? 'checkmark-outline' : 'copy'" :size="16"
          /></template>
