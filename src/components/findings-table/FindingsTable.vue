...
const GRID_COLS = "grid-cols-[40px_24px_minmax(0,1fr)_260px_160px] gap-3";
...
<template>
  <div class="w-full overflow-x-auto">
    <div class="min-w-[1118px]">
      <div :class="['sticky top-0 z-10 grid items-center bg-[#f4f4f4]', GRID_COLS]">
        <span />
        <Checkbox :model-value="allSelected()" @update:model-value="emit('toggle-all', $event)" />
        <span class="py-4 pr-2 pl-1 text-[14px] leading-[18px] font-semibold tracking-[0.16px] text-[#525252]">File path</span>
        <span class="py-4 pr-2 pl-1 text-[14px] leading-[18px] font-semibold tracking-[0.16px] text-[#525252]">Findings</span>
        <span class="py-4 pr-2 pl-1 text-[14px] leading-[18px] font-semibold tracking-[0.16px] text-[#525252]">Status</span>
      </div>
      ...
      <div v-for="file in files" :key="file.filename" class="border-b border-[#e0e0e0]">
        ...
        >
          <div v-if="hasFindings(file)" class="grid grid-cols-3 gap-10 bg-white p-4">
